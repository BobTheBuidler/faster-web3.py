...
@pytest.mark.parametrize(
    "abi_type,expected", array_subtype_cases, ids=["simple", "nested"]
)
...
@pytest.mark.parametrize(
    "abi_type,expected", array_type_cases, ids=["fixed", "nested", "dynamic"]
)
...
@pytest.mark.parametrize(
    "s,expected", tuple_type_str_cases, ids=["plain", "one-dim", "two-dim", "not-tuple"]
)
...
def test_get_name_from_abi_element_identifier(
    benchmark: BenchmarkFixture, identifier, expected
):
...
def test_faster_get_name_from_abi_element_identifier(
    benchmark: BenchmarkFixture, identifier, expected
):
    benchmark(
        run_100, faster_web3._utils.abi.get_name_from_abi_element_identifier, identifier
    )
...
@pytest.mark.parametrize(
    "identifier,arg_types,expected",
    element_sig_cases,
    ids=["transfer", "fallback", "receive", "custom"],
)
def test_get_abi_element_signature(
    benchmark: BenchmarkFixture, identifier, arg_types, expected
):
...
def test_faster_get_abi_element_signature(
    benchmark: BenchmarkFixture, identifier, arg_types, expected
):
    benchmark(
        run_100, faster_web3._utils.abi.get_abi_element_signature, identifier, arg_types
    )
...
example_abi = [
    {"type": "function", "name": "foo", "inputs": [{"name": "a", "type": "uint256"}]},
    {
        "type": "event",
        "name": "Bar",
        "inputs": [{"name": "b", "type": "address", "indexed": True}],
    },
...
@pytest.mark.parametrize(
    "arg_names,expected_count", by_arg_name, ids=["a", "b", "c", "notfound", "a+b"]
)
def test_filter_by_argument_name(
    benchmark: BenchmarkFixture, arg_names, expected_count
):
    benchmark(
        run_100, web3._utils.abi.filter_by_argument_name, arg_names, function_only_abi
    )
...
def test_faster_filter_by_argument_name(
    benchmark: BenchmarkFixture, arg_names, expected_count
):
    benchmark(
        run_100,
        faster_web3._utils.abi.filter_by_argument_name,
        arg_names,
        function_only_abi,
    )
...
@pytest.mark.parametrize(
    "arg_types,expected_count",
    by_arg_type,
    ids=["uint256", "address", "string", "notatype", "uint+address"],
)
def test_filter_by_argument_type(
    benchmark: BenchmarkFixture, arg_types, expected_count
):
...
def test_faster_filter_by_argument_type(
    benchmark: BenchmarkFixture, arg_types, expected_count
):
    benchmark(
        run_100, faster_web3._utils.abi.filter_by_argument_type, arg_types, example_abi
    )
...
def test_find_constructor_abi_element_by_type(benchmark: BenchmarkFixture):
    benchmark(
        run_100, web3._utils.abi.find_constructor_abi_element_by_type, example_abi
    )
...
def test_faster_find_constructor_abi_element_by_type(benchmark: BenchmarkFixture):
    benchmark(
        run_100,
        faster_web3._utils.abi.find_constructor_abi_element_by_type,
        example_abi,
    )
