...
property_factory_cases = [
    ...
    (
        "BigClassInvalid",
        (type,),
        {**{f"x{i}": i for i in range(100)}, "notfound": 1},
        None,
    ),
...
@pytest.mark.parametrize(
    "name,bases,namespace,normalizers", property_factory_cases, ids=property_factory_ids
)
...
def test_faster_PropertyCheckingFactory(
    benchmark: BenchmarkFixture, name, bases, namespace, normalizers
):
    benchmark(
