...
HEX_TO_INTEGER_CASES = [
    ...
    "-0x1a",          # negative hex
    "not_a_hex",      # non-hex string
    "0XABCDEF",       # uppercase hex
    "0x123",          # odd length hex
]
HEX_TO_INTEGER_IDS = [
    "realistic_hex", "zero_hex", "empty_hex_prefix", "no_prefix",
    "large_hex", "negative_hex", "non_hex", "uppercase_hex", "odd_length_hex"
]

APPLY_FORMATTERS_TO_ARGS_CASES = [
    (INT_VAL, str(INT_VAL)),
    (INT_VAL,),
    (),  # no args
    (None, 123),  # with None
    (123, "abc", None),  # mixed types
]
APPLY_FORMATTERS_TO_ARGS_IDS = [
    "two_args", "one_arg", "no_args", "with_none", "mixed_types"
]
...
MAP_COLLECTION_IDS = [
    "real_topics", "empty_list", "transaction_dict", "string_input",
    "nested_list", "set_input", "tuple_input", "dict_nonstring_keys"
]
...
def test_faster_apply_error_formatters(benchmark: BenchmarkFixture, response):
    benchmark(run_100, faster_web3._utils.formatters.apply_error_formatters, None, response)
...
def test_apply_null_result_formatters(benchmark: BenchmarkFixture, response):
    benchmark(run_100, web3._utils.formatters.apply_null_result_formatters, None, response)
...
def test_faster_apply_null_result_formatters(benchmark: BenchmarkFixture, response):
    benchmark(run_100, faster_web3._utils.formatters.apply_null_result_formatters, None, response)
