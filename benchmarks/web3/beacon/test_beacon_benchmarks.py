...
def test_get_hash_root(benchmark: BenchmarkFixture, state_id):
    beacon = web3.beacon.Beacon(base_url="http://localhost:5052")
    with patch(
        "requests.Session.get", side_effect=make_fake_send({"hash_root": state_id})
    ):
...
def test_get_fork_data(benchmark: BenchmarkFixture, state_id):
    beacon = web3.beacon.Beacon(base_url="http://localhost:5052")
    with patch(
        "requests.Session.get", side_effect=make_fake_send({"fork_data": state_id})
    ):
...
def test_get_finality_checkpoint(benchmark: BenchmarkFixture, state_id):
    ...
    with patch(
        ...
        side_effect=make_fake_send({"finality_checkpoint": state_id}),
    ):
...
def test_get_validators(benchmark: BenchmarkFixture, state_id):
    beacon = web3.beacon.Beacon(base_url="http://localhost:5052")
    with patch(
        "requests.Session.get", side_effect=make_fake_send({"validators": state_id})
    ):
...
def test_get_validator(benchmark: BenchmarkFixture, validator_id, state_id):
    ...
    with patch(
        "requests.Session.get",
        side_effect=make_fake_send({"validator": validator_id, "state": state_id}),
    ):
...
def test_get_validator_balances(benchmark: BenchmarkFixture, state_id):
    ...
    with patch(
        "requests.Session.get",
        side_effect=make_fake_send({"validator_balances": state_id}),
    ):
...
def test_get_epoch_committees(benchmark: BenchmarkFixture, state_id):
    ...
    with patch(
        "requests.Session.get",
        side_effect=make_fake_send({"epoch_committees": state_id}),
    ):
...
def test_get_epoch_sync_committees(benchmark: BenchmarkFixture, state_id):
    ...
    with patch(
        "requests.Session.get",
        side_effect=make_fake_send({"epoch_sync_committees": state_id}),
    ):
...
def test_get_epoch_randao(benchmark: BenchmarkFixture, state_id):
    beacon = web3.beacon.Beacon(base_url="http://localhost:5052")
    with patch(
        "requests.Session.get", side_effect=make_fake_send({"epoch_randao": state_id})
    ):
...
def test_get_block_headers(benchmark: BenchmarkFixture):
    beacon = web3.beacon.Beacon(base_url="http://localhost:5052")
    with patch(
        "requests.Session.get", side_effect=make_fake_send({"block_headers": True})
    ):
...
def test_get_block_header(benchmark: BenchmarkFixture, block_id):
    beacon = web3.beacon.Beacon(base_url="http://localhost:5052")
    with patch(
        "requests.Session.get", side_effect=make_fake_send({"block_header": block_id})
    ):
...
def test_get_block_root(benchmark: BenchmarkFixture, block_id):
    beacon = web3.beacon.Beacon(base_url="http://localhost:5052")
    with patch(
        "requests.Session.get", side_effect=make_fake_send({"block_root": block_id})
    ):
...
def test_get_block_attestations(benchmark: BenchmarkFixture, block_id):
    ...
    with patch(
        "requests.Session.get",
        side_effect=make_fake_send({"block_attestations": block_id}),
    ):
...
def test_get_blinded_blocks(benchmark: BenchmarkFixture, block_id):
    beacon = web3.beacon.Beacon(base_url="http://localhost:5052")
    with patch(
        "requests.Session.get", side_effect=make_fake_send({"blinded_blocks": block_id})
    ):
...
def test_get_rewards(benchmark: BenchmarkFixture, block_id):
    beacon = web3.beacon.Beacon(base_url="http://localhost:5052")
    with patch(
        "requests.Session.get", side_effect=make_fake_send({"rewards": block_id})
    ):
...
def test_get_light_client_bootstrap_structure(benchmark: BenchmarkFixture, block_root):
    ...
    with patch(
        "requests.Session.get",
        side_effect=make_fake_send({"light_client_bootstrap_structure": block_root}),
    ):
...
def test_get_light_client_updates(benchmark: BenchmarkFixture):
    ...
    with patch(
        "requests.Session.get",
        side_effect=make_fake_send({"light_client_updates": True}),
    ):
...
def test_get_light_client_finality_update(benchmark: BenchmarkFixture):
    ...
    with patch(
        "requests.Session.get",
        side_effect=make_fake_send({"light_client_finality_update": True}),
    ):
...
def test_get_light_client_optimistic_update(benchmark: BenchmarkFixture):
    ...
    with patch(
        "requests.Session.get",
        side_effect=make_fake_send({"light_client_optimistic_update": True}),
    ):
...
def test_get_attestations(benchmark: BenchmarkFixture):
    beacon = web3.beacon.Beacon(base_url="http://localhost:5052")
    with patch(
        "requests.Session.get", side_effect=make_fake_send({"attestations": True})
    ):
...
def test_get_attester_slashings(benchmark: BenchmarkFixture):
    beacon = web3.beacon.Beacon(base_url="http://localhost:5052")
    with patch(
        "requests.Session.get", side_effect=make_fake_send({"attester_slashings": True})
    ):
...
def test_get_proposer_slashings(benchmark: BenchmarkFixture):
    beacon = web3.beacon.Beacon(base_url="http://localhost:5052")
    with patch(
        "requests.Session.get", side_effect=make_fake_send({"proposer_slashings": True})
    ):
...
def test_get_voluntary_exits(benchmark: BenchmarkFixture):
    beacon = web3.beacon.Beacon(base_url="http://localhost:5052")
    with patch(
        "requests.Session.get", side_effect=make_fake_send({"voluntary_exits": True})
    ):
...
def test_get_bls_to_execution_changes(benchmark: BenchmarkFixture):
    ...
    with patch(
        "requests.Session.get",
        side_effect=make_fake_send({"bls_to_execution_changes": True}),
    ):
...
def test_get_fork_schedule(benchmark: BenchmarkFixture):
    beacon = web3.beacon.Beacon(base_url="http://localhost:5052")
    with patch(
        "requests.Session.get", side_effect=make_fake_send({"fork_schedule": True})
    ):
...
def test_get_deposit_contract(benchmark: BenchmarkFixture):
    beacon = web3.beacon.Beacon(base_url="http://localhost:5052")
    with patch(
        "requests.Session.get", side_effect=make_fake_send({"deposit_contract": True})
    ):
...
def test_get_beacon_state(benchmark: BenchmarkFixture, state_id):
    beacon = web3.beacon.Beacon(base_url="http://localhost:5052")
    with patch(
        "requests.Session.get", side_effect=make_fake_send({"beacon_state": state_id})
    ):
...
def test_get_beacon_heads(benchmark: BenchmarkFixture):
    beacon = web3.beacon.Beacon(base_url="http://localhost:5052")
    with patch(
        "requests.Session.get", side_effect=make_fake_send({"beacon_heads": True})
    ):
...
def test_get_node_identity(benchmark: BenchmarkFixture):
    beacon = web3.beacon.Beacon(base_url="http://localhost:5052")
    with patch(
        "requests.Session.get", side_effect=make_fake_send({"node_identity": True})
    ):
...
def test_get_peer_count(benchmark: BenchmarkFixture):
    beacon = web3.beacon.Beacon(base_url="http://localhost:5052")
    with patch(
        "requests.Session.get", side_effect=make_fake_send({"peer_count": True})
    ):
...
def test_get_blob_sidecars(benchmark: BenchmarkFixture, block_id, indices):
    ...
    with patch(
        "requests.Session.get",
        side_effect=make_fake_send({"block": block_id, "indices": indices}),
    ):
...
def test_get_block_proposer_duties(benchmark: BenchmarkFixture, epoch):
    ...
    with patch(
        "requests.Session.get",
        side_effect=make_fake_send({"block_proposer_duties": epoch}),
    ):
