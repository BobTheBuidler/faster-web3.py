...
PyInit_contract_sources(void)
{
    ...
    tmp = PyImport_ImportModuleLevel("faster_web3_contract_source_packages__mypyc", NULL, NULL, fromlist, 0);
    if (!tmp) return NULL;
    ...
    if (deps_capsule != NULL) {
        int (*deps_func)(void) = (int (*)(void))PyCapsule_GetPointer(
            deps_capsule, "faster_web3_contract_source_packages__mypyc.ensure_deps");
...
PyMODINIT_FUNC PyInit___init__(void) { return PyInit_contract_sources(); }
