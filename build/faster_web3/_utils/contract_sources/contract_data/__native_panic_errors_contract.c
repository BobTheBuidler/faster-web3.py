...
PyObject *CPyInitOnly_faster_web3____utils___contract_sources___contract_data___panic_errors_contract(void)
{
...
PyObject *CPyInit_faster_web3____utils___contract_sources___contract_data___panic_errors_contract(void)
{
    ...
    if (unlikely(CPyModule_faster_web3____utils___contract_sources___contract_data___panic_errors_contract__internal == NULL))
        goto fail;
    modname = PyUnicode_FromString("faster_web3._utils.contract_sources.contract_data.panic_errors_contract");
    if (modname == NULL) CPyError_OutOfMemory();
    int rv = 0;
    PyObject *mod_dict = PyImport_GetModuleDict();
    PyObject *shared_lib = NULL;
    rv = PyDict_GetItemStringRef(mod_dict, "faster_web3._utils.contract_sources.contract_data.panic_errors_contract__mypyc", &shared_lib);
    if (rv < 0) goto fail;
    ...
    Py_ssize_t is_pkg = 0;
    rv = CPyImport_SetDunderAttrs(CPyModule_faster_web3____utils___contract_sources___contract_data___panic_errors_contract__internal, modname, shared_lib_file, ext_suffix, is_pkg);
    Py_DECREF(ext_suffix);
    Py_DECREF(shared_lib_file);
    if (rv < 0) goto fail;
    if (PyObject_SetItem(PyImport_GetModuleDict(), modname, CPyModule_faster_web3____utils___contract_sources___contract_data___panic_errors_contract__internal) < 0)
        goto fail;
    Py_CLEAR(modname);
...
CPyL3: ;
    ...
    *(PyObject * *)cpy_r_r76 = cpy_r_r73;
    ...
    cpy_r_r95 = (CPyPtr)((CPyPtr)cpy_r_r94 + offsetof(PyListObject, ob_item));
    ...
    *(PyObject * *)cpy_r_r96 = cpy_r_r93;
    ...
    cpy_r_r174 = (CPyPtr)((CPyPtr)cpy_r_r173 + offsetof(PyListObject, ob_item));
