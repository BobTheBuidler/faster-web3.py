...
PyInit_tuple_contracts(void)
{
    ...
    tmp = PyImport_ImportModuleLevel("faster_web3._utils.contract_sources.contract_data.tuple_contracts__mypyc", NULL, NULL, fromlist, 0);
    if (!tmp) return NULL;
    ...
    if (deps_capsule != NULL) {
        int (*deps_func)(void) = (int (*)(void))PyCapsule_GetPointer(
            deps_capsule, "faster_web3._utils.contract_sources.contract_data.tuple_contracts__mypyc.ensure_deps");
    ...
    PyObject *capsule = PyObject_GetAttrString(tmp, "init_faster_web3____utils___contract_sources___contract_data___tuple_contracts");
    Py_DECREF(tmp);
    if (capsule == NULL) return NULL;
    void *init_func = PyCapsule_GetPointer(capsule, "faster_web3._utils.contract_sources.contract_data.tuple_contracts__mypyc.init_faster_web3____utils___contract_sources___contract_data___tuple_contracts");
    Py_DECREF(capsule);
    if (!init_func) {
        return NULL;
    }
...
PyMODINIT_FUNC PyInit___init__(void) { return PyInit_tuple_contracts(); }
