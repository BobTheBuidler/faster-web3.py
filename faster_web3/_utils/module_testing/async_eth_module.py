...
if TYPE_CHECKING:
    ...
    from faster_web3.main import AsyncWeb3, Web3
...
class AsyncEthModuleTest:
    ...
    async def test_eth_send_transaction_legacy(
        ...
        txn_params: TxParams = {
            ...
            "value": Wei(1),
            "gas": 21000,
            "gasPrice": await eth.gas_price,
        }
        txn_hash = await eth.send_transaction(txn_params)
        txn = await eth.get_transaction(txn_hash)
    ...
    async def test_eth_modify_transaction_legacy(
        ...
        modified_txn_hash = await eth.modify_transaction(
            txn_hash, gasPrice=(cast(Wei, txn_params["gasPrice"] * 2)), value=Wei(2)
        )
        modified_txn = await eth.get_transaction(modified_txn_hash)
    ...
    async def test_async_eth_sign_transaction(
        ...
        eth = async_w3.eth
        result = await eth.sign_transaction(txn_params)
        signatory_account = eth.account.recover_transaction(result["raw"])
    ...
    async def test_validation_middleware_chain_id_mismatch(
        ...
        eth = async_w3.eth
        actual_chain_id = await eth.chain_id
        ...
        with pytest.raises(
            ...
            await eth.send_transaction(txn_params)
    ...
    async def test_async_eth_send_raw_transaction(
        ...
        eth = async_w3.eth
        keyfile_account = eth.account.from_key(keyfile_account_pkey)
        txn = {
            ...
            "value": Wei(0),
            "gas": 21000,
            "nonce": await eth.get_transaction_count(
                keyfile_account.address, "pending"
            ),
            "gasPrice": 10**9,
        }
        signed = keyfile_account.sign_transaction(txn)
        txn_hash = await eth.send_raw_transaction(signed.raw_transaction)
    ...
    async def test_async_sign_and_send_raw_middleware(
        ...
        txn_hash = await eth.send_transaction(txn)
    ...
    async def test_async_sign_authorization_send_raw_and_send_set_code_transactions(
        ...
        nonce = await eth.get_transaction_count(keyfile_account.address)
        ...
        signed = keyfile_account.sign_transaction(txn)
        tx_hash = await eth.send_raw_transaction(signed.raw_transaction)
        get_tx = await eth.get_transaction(tx_hash)
        await eth.wait_for_transaction_receipt(tx_hash)

        code = await eth.get_code(keyfile_account.address)
        assert code.to_0x_hex() == f"0xef0100{async_math_contract.address[2:].lower()}"
        delegated = eth.contract(
        ...
        reset_tx_hash = await eth.send_transaction(reset_code_txn)
        await eth.wait_for_transaction_receipt(reset_tx_hash, timeout=10)

        reset_code = await eth.get_code(keyfile_account.address)
    ...
    async def test_GasPriceStrategyMiddleware(
        ...
        eth.set_gas_price_strategy(None)  # reset strategy
    ...
    async def test_gas_price_strategy_middleware_hex_value(
        ...
        eth = async_w3.eth
        eth.set_gas_price_strategy(gas_price_strategy)  # type: ignore
    ...
    async def test_gas_price_from_strategy_bypassed_for_dynamic_fee_txn(
        ...
        latest_block = await eth.get_block("latest")
        assert (
    ...
    async def test_eth_getBlockByNumber_earliest(
        ...
        eth = async_w3.eth
        genesis_block = await eth.get_block(BlockNumber(0))
        block = await eth.get_block("earliest")
    ...
    async def test_eth_get_raw_transaction_by_block(
        ...
        async def wait_for_block_with_txn() -> HexBytes:
            while True:
                try:
                    return await eth.get_raw_transaction_by_block("latest", 0)
        ...
        raw_transaction = await eth.get_raw_transaction_by_block(
    ...
    async def test_eth_get_raw_transaction_by_block_raises_error_block_identifier(
        ...
        with pytest.raises(
            ...
            await eth.get_raw_transaction_by_block(unknown_identifier, 0)  # type: ignore  # noqa: E501
    ...
    async def test_eth_get_balance(self, async_w3: "AsyncWeb3") -> None:
        ...
        with pytest.raises(InvalidAddress):
            await eth.get_balance(
                ChecksumAddress(HexAddress(HexStr(account.lower())))
            )

        balance = await eth.get_balance(account)
    ...
    async def test_eth_get_code_with_block_identifier(
        ...
        eth = async_w3.eth
        block_id = await eth.block_number
        code = await eth.get_code(async_emitter_contract.address, block_id)
        assert isinstance(code, HexBytes)
        assert len(code) > 0
    ...
    async def test_eth_create_access_list(
        ...
        # create access list
        eth = async_w3.eth
        response = await eth.create_access_list(txn)
        ...
        await eth.send_transaction(txn)
    ...
    async def test_eth_call(
        ...
        call_result = await eth.call(txn_params)
    ...
    async def test_eth_call_with_override_code(
        ...
        bytes_call_result = await eth.call(
            txn_params,
            "latest",
    ...
    async def test_async_eth_get_transaction_receipt_unmined(
        ...
        eth = async_w3.eth
        txn_hash = await eth.send_transaction(
            {
                "from": async_keyfile_account_address_dual_type,
                "to": async_keyfile_account_address_dual_type,
                "value": Wei(1),
        ...
        with pytest.raises(TransactionNotFound):
            await eth.get_transaction_receipt(txn_hash)
    ...
    async def test_async_eth_wait_for_transaction_receipt_unmined(
        ...
        with pytest.raises(TimeExhausted) as exc_info:
            await eth.wait_for_transaction_receipt(txn_hash, timeout=timeout)
    ...
    async def test_async_eth_get_logs_without_logs(
        ...
        result = await eth.get_logs(filter_params)
        assert len(result) == 0
    ...
    async def test_async_eth_get_storage_at(
        ...
        slot_0 = await eth.get_storage_at(async_storage_contract_address, 0)
        assert slot_0 == HexBytes(f"0x{'00' * 32}")

        slot_1 = await eth.get_storage_at(async_storage_contract_address, 1)
        assert slot_1 == HexBytes(f"0x{'00' * 31}01")

        slot_2 = await eth.get_storage_at(async_storage_contract_address, 2)
        assert slot_2 == HexBytes(f"0x{'00' * 31}02")

        slot_3 = await eth.get_storage_at(async_storage_contract_address, 3)
        assert slot_3 == HexBytes(
            "0x746872656500000000000000000000000000000000000000000000000000000a"
        )
        assert bytes(slot_3[:5]) == b"three"

        slot_4 = await eth.get_storage_at(async_storage_contract_address, 4)
    ...
    def test_async_provider_default_account(
        ...
        eth = async_w3.eth
        current_default_account = eth.default_account

        # check setter
        eth.default_account = async_keyfile_account_address_dual_type
        default_account = eth.default_account
        assert default_account == async_keyfile_account_address_dual_type

        # reset to default
        eth.default_account = current_default_account
    ...
    def test_async_provider_default_block(
        ...
        default_block = eth.default_block
        assert default_block == "latest"

        # check setter
        eth.default_block = BlockNumber(12345)
        default_block = eth.default_block
        assert default_block == BlockNumber(12345)

        # reset to default
        eth.default_block = "latest"
    ...
    async def test_async_eth_replace_transaction_legacy(
        ...
        replace_txn_hash = await eth.replace_transaction(txn_hash, txn_params)
        replace_txn = await eth.get_transaction(replace_txn_hash)
    ...
    async def test_async_eth_replace_transaction_underpriced(
        ...
        with pytest.raises(Web3RPCError, match="replacement transaction underpriced"):
            await eth.replace_transaction(txn_hash, txn_params)
    ...
    async def test_async_eth_replace_transaction_already_mined(
        ...
        eth = async_w3.eth
        txn_hash = await eth.send_transaction(txn_params)
        await eth.wait_for_transaction_receipt(txn_hash, timeout=10)
    ...
    async def test_async_eth_replace_transaction_gas_price_defaulting_strategy_higher(
        ...
        eth.set_gas_price_strategy(higher_gas_price_strategy)
    ...
    async def test_async_eth_replace_transaction_gas_price_defaulting_strategy_lower(
        ...
        eth.set_gas_price_strategy(lower_gas_price_strategy)
    ...
    async def test_async_eth_new_filter(self, async_w3: "AsyncWeb3") -> None:
        eth = async_w3.eth
        filter = await eth.filter({})

        changes = await eth.get_filter_changes(filter.filter_id)
        assert is_list_like(changes)
        assert not changes

        logs = await eth.get_filter_logs(filter.filter_id)
        assert is_list_like(logs)
        assert not logs

        result = await eth.uninstall_filter(filter.filter_id)
        assert result is True

    @pytest.mark.asyncio
    async def test_async_eth_new_block_filter(self, async_w3: "AsyncWeb3") -> None:
        eth = async_w3.eth
        filter = await eth.filter("latest")
    ...
    async def test_async_eth_new_pending_transaction_filter(
        ...
        eth = async_w3.eth
        filter = await eth.filter("pending")
        assert is_string(filter.filter_id)
    ...
    async def test_async_eth_uninstall_filter(self, async_w3: "AsyncWeb3") -> None:
        ...
        success = await eth.uninstall_filter(filter.filter_id)
        assert success is True

        failure = await eth.uninstall_filter(filter.filter_id)
